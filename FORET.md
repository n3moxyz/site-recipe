...
- Sharing: a custom 1200 × 630 social card lives at `public/og.jpg`; `public/og.png` preserves the URL captured by the hackathon gallery. A recipe can also be shared via a URL hash that restores every choice.
...
- Part 4 selections render as recognizable miniature UI specimens in catalog order—bento, tabs, accordion, carousel, dialog, and command palette—with a six-item cap, a plain-list specimen for "Keep it plain", and reduced-motion-safe entry feedback.
...
- **The gallery preview URL outlived the file extension.** The hackathon gallery stored the site's original `/og.png` cover URL. A later optimization replaced the PNG with a smaller `/og.jpg` and updated the page metadata, but the gallery did not re-scrape the new URL, so its card kept requesting a 404 and showed “Preview unavailable.” Public asset URLs can become external contracts even when they are not application routes. Keep the legacy `og.png` compatibility asset alongside the canonical JPEG, and cover both files with a dimensions-and-signature regression test.
